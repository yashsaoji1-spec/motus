...
- **Cloudinary** — session video storage; unsigned uploads via `phalanx-videos` preset to cloud `dslbugsdg`; free tier 25 GB. Constants `CLOUDINARY_CLOUD` / `CLOUDINARY_PRESET` at top of Section 1. Bitrate: **400 kbps** video + ~64 kbps audio (~3.5 MB/min). At typical 5-min sessions (~17.5 MB each), the 25 GB free tier holds ~1,400 sessions.
...
| `customExercises` | auto-id                    | `{ id, name, cat, dr, ds, df, desc, createdBy }` — legacy global custom exercises; loaded and merged into `PROTOCOL_CATALOG` on `openAddProtocol`; `id` is a slug derived from `name` |
| `therapistLibrary` | `{therapistEmail}`         | `{ customExercises: [{ id, name, cat, dr, ds, df, desc, createdAt }, …], hiddenIds: [id, …], editedBuiltIns: [{ id, name, cat, dr, ds, df, desc }, …] }` — per-therapist protocol customizations; `hiddenIds` hides exercises from their view; `editedBuiltIns` stores shadow copies of overridden built-in exercises; `customExercises` stores therapist-created exercises |
...
- Sidebar: patient list with search, clinic code copy button (blue badge), phalanX logo + title, Protocol Library button
- Main panel (empty state): shows "Bulk Assign" button in header
- When patient selected: shows back button (←) in patient header to deselect, "Add Protocol" button
- Clicking "Bulk Assign" opens bulk assign modal with patient search
- Clicking "Protocol Library" opens protocol library modal (exercise management)
